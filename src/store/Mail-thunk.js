  ...
  return async (Disptach) => {
    ...
    const sendingmail = async () => {
      const response = await fetch(
        `https://mail-box-client-bbf63-default-rtdb.firebaseio.com/${emailId}/inbox.json`,
        {
          method: "POST",
...
export const getmailHandler = () => {
  ...
  return async (Disptach) => {
    const gettingMailList = async () => {
      const response = await fetch(
        `https://mail-box-client-bbf63-default-rtdb.firebaseio.com/${emailId}.json`,
        {
          method: "GET",
        }
      );
...
export const UpdateList = (obj) => {
  return async (Dispatch) => {
    ...
    const UpdateEmailList = async () => {
      const response = await fetch(
        `https://mail-box-client-bbf63-default-rtdb.firebaseio.com/${emailId}/inbox/${obj.id}.json`,
        {
          method: "PUT",
          body: JSON.stringify({
            email: obj.email,
...
export const DeleteMail = (id) => {
  return async (Dispatch) => {
    ...
    const DeletingMail = async () => {
      const response = await fetch(
        `https://mail-box-client-bbf63-default-rtdb.firebaseio.com/${emailId}/inbox/${id}.json`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
...
export const UpdateMySentItem = (sentItem) => {
  return async (Dispatch) => {
    ...
    const UpdatedingmySendingItem = async () => {
      const response = await fetch(
        `https://mail-box-client-bbf63-default-rtdb.firebaseio.com/${emailId}/sentItem/.json`,
        {
          method: "PUT",
